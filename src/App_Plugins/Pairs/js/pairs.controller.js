angular.module("umbraco").controller("Threewood.Umbraco.PairsController",
function ($scope, $filter) {
    ...
    else
    {
        if (angular.isObject($scope.model.value)) {
            $scope.model.selected = $filter('filter')($scope.model.config.items, $scope.model.value.key)[0];
        }
    }
    ...
    //    //$scope.model.value = JSON.stringify(v);
    //    //$scope.selected = "SAM";
    //});

    $scope.$watch('model.selected', function (v) {
        if ($scope.model.config.developerMode) {
            console.log(v);
        }
